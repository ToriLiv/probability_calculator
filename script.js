...
//funcion para cal
function factorial(n) {//calcula cuántas formas existen de obtener x éxitos en n intentos
  if (n < 0) {
...
function probabilidadBinomial(x, n, p) {
  porcentaje = p / 100;
  const coeficienteBinomial = factorial(n) / (factorial(x) * factorial(n - x)); //----->calcular la ombinada
  const probabilidad =
...
submitBtn.addEventListener("click", (e) => {
  ...
  if (isNaN(n) || isNaN(x) || isNaN(p)) {
    ...
    return;
  } else if (n < 0 || x < 0 || p < 0) { //validar que n, x y p no sean negativos
    Swal.fire({
      title: "Número inválido",
      text: `⚠️ No puede ingresar números negativos`,
    });
    return;
  } else if (n % 1 !== 0 || x % 1 !== 0 || p % 1 !== 0) { //validar que p, x y n sean enteros
    Swal.fire({
      title: "Número decimal",
      text: `⚠️ Todos los valores ingresados deben ser números enteros`,
    });
    return;
  } else if (p < 0 || p > 100) { //validar que el rango de p sea entre 0 y 100
    Swal.fire({
      title: "Porcentaje inválido",
      text: `⚠️ La probabilidad debe estar entre 0 y 100`,
    });
    return;
  } else if (x > n) { //validar que los casos de exito no sean mayorea al numero de experimentos
    Swal.fire({
